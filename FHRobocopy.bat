...
@echo off
del *.txt
title FlashcartHelper v0.5 Robocopy version: GPL notice
...
pause
cls
title FlashcartHelper v0.5 Robocopy version
echo This is the Robocopy Compliant version
echo This will not work with Windows XP or below without the Windows Resource kit
echo availible at http://www.microsoft.com/download/en/details.aspx?id=17657
echo If you do not wish to install the Windows Resource kit, please use the Xcopy version.
pause
:wgettest
IF EXIST wget.exe. (
goto unrartest
...
echo FlashcartHelper is unable to continue
...
echo.
echo [2]Panasonic formatter (Format my MicroSD)
echo.
...
echo.
echo [6]eNDecryptS advance(To decrypt roms)
echo.
...
echo Please wait, this may take a while..
wgest http://filetrip.net/h35131424-CATSFC.html
7za x *.zip -x!version -x!source.txt -x!copyright -x!installation.txt
robocopy %cd%\CATSFC %cd%\Put_This_In_SD_card\CATSFC /s /e
copy %cd%\*.ini %cd%\Put_This_In_SD_card\_dstwoplug\*.ini
copy %cd%\*.bmp %cd%\Put_This_In_SD_card\_dstwoplug\*.bmp
copy %cd%\*.plg %cd%\Put_This_In_SD_card\_dstwoplug\*.plg
rmdir %cd%\CATSFC\ /S /Q
del %cd%\*.ini
del %cd%\*.plg
del %cd%\*.bmp
...
7za x *.zip
robocopy 201002161705_moonshell210stable %cd%\Put_This_In_SD_card\ /s /e 
